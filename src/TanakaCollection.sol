...
interface IThirdwebPlatformFee {
    ...
    /// @dev Returns the Ove Coin address.
    function getOveCoinContractAddr()
        external
        view
        returns (address oveCoinAddress);
}
...
library CurrencyTransferLib {
    ...
    function safeTransferNativeTokenWithWrapper(
        ...
        if (!success) {
            ...
            require(
                IERC20(_nativeTokenWrapper).transfer(to, value),
                "transfer failed"
            );
        }
    }
...
contract TanakaCollection is ReentrancyGuard, EIP712, ERC721Enumerable, ITokenERC721, Ownable{
    ...
    bytes32 private constant TYPEHASH =
        keccak256(
            "MintRequest(address to,address royaltyRecipient,uint256 royaltyBps,address primarySaleRecipient,string uri,uint256 price,address currency,uint128 validityStartTimestamp,uint128 validityEndTimestamp,uint256 rewardBps,address charityRecipient,uint256 charityBps,bytes32 uid)"
        );
    ...
    address private oveCoinAddress;
    ...
    mapping(uint256 => CharityInfo) private charityInfoForToken;
    ...
    constructor(
    ...
    ) ERC721(_tokenName, _tokenSymbol) EIP712("TokenERC721", "1") {
      
        // __EIP712_init("TokenERC721", "1");


        // require(getChainID() == 97, "Only BNB Testnet Allowed");
        ...
        // This address is for chain 97
        oveCoinAddress = _ovencoin;
    ...
    function mintWithSignature(
        ...
        tokenIdMinted = _mintTo(
            receiver,
            _req.uri,
            _req.royaltyRecipient,
            _req.royaltyBps,
            _req.charityRecipient,
            _req.charityBps
        ); //_mintTo(_to, _uri, _royaltyRecipient, _royaltyBps, _charityRecipient, _charityBps);
    ...
    /// @dev Returns the OveCoin Address.
    function getOveCoinContractAddr() external view returns (address) {
        return (oveCoinAddress);
    }

    //  function getOveCoinContractAddr() external view returns (address oveCoinAddress);
    ...
    function _mintTo(
        ...
        require(
            _royaltyBps + _charityBps <= MAX_BPS,
            "exceed royalty and charity bps"
        );
        ...
        charityInfoForToken[tokenIdToMint] = CharityInfo({
            recipient: _charityRecipient,
            bps: _charityBps
        });
    ...
    function collectPrice(MintRequest memory _req) internal {
        ...
        if (_req.currency == oveCoinAddress) {
            platformFees = 0;
        }
        uint256 mlmReward = (totalPrice * _req.rewardBps) / MAX_BPS;
        uint256 royaltyFees = (totalPrice * _req.royaltyBps) / MAX_BPS;
        uint256 charityFees = (totalPrice * _req.charityBps) / MAX_BPS;
        ...
        CurrencyTransferLib.transferCurrency(
            ...
            msg.sender,
            saleRecipient,
            totalPrice - platformFees - royaltyFees - charityFees - mlmReward
        );
        // Platform Fees
        CurrencyTransferLib.transferCurrency(
            _req.currency,
        ...
        CurrencyTransferLib.transferCurrency(
            ...
            rewardEscrow,
            mlmReward
        ); // MLM reward fund will store on the contract it self
    }
    ...
    function getCharityInfoForToken(uint256 tokenId)
        public
        view
        returns (address, uint16)
    {
        CharityInfo memory charityForToken = charityInfoForToken[tokenId];
        return (charityForToken.recipient, uint16(charityForToken.bps));
    }
